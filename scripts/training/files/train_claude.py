...
def build_model(config: LlamaConfig, init_std: float = 0.02) -> LlamaForCausalLM:
    ...
    def _init_weights(module):
        if isinstance(module, nn.Embedding):
            ...
            # Determine if this is a residual projection by checking the
            # attribute name on the parent — walk named_modules for the key.
            std = init_std
            for name, mod in module.named_parameters(recurse=False):
                pass  # just need to enter the branch
            # Check module's own name via the parent walk done at apply() time;
            # we rely on a name-tagged wrapper instead.
...
def main():
    ...
    eval_steps = max(50, min(500, total_steps // 20))
