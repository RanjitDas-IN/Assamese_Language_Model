    ...
    def load(self) -> dict:
        if not self.path.exists():
            return {
                ...
                "completed_files": {},
                "writers": {},
                "stats": {"files_processed": 0, "tokens_written": 0},
                "in_progress": None,
            }
        state = json.loads(self.path.read_text(encoding="utf-8"))
        if state.get("version") != STATE_VERSION:
            raise RuntimeError("State version mismatch; use a fresh state file")
        return state
...
class ShardWriter:
    ...
    @property
    def shard_token_capacity(self) -> int:
        return SHARD_SIZE_BYTES // UINT16_BYTES

    def _bootstrap_or_recover(self) -> None:
        ws = self.state["writers"][self.split]
        if not ws:
            ws.update({"current_index": 0, "tokens_in_current_shard": 0, "records": []})

        self.current_index = int(ws["current_index"])
        self.tokens_in_current_shard = int(ws["tokens_in_current_shard"])
    ...
    def _sync_state(self) -> None:
        ws = self.state["writers"][self.split]
        ws["current_index"] = self.current_index
        ws["tokens_in_current_shard"] = self.tokens_in_current_shard
        ws["records"] = self.records
    ...
    def _ensure_record(self, idx: int) -> dict:
        while len(self.records) <= idx:
            ...
            self.records.append(
                ShardRecord(
                    index=i,
                    filename=f"{self.split}_{i:03d}.bin",
                    split=self.split,
                ).__dict__
            )
    ...
    def _flush_rolling(self) -> int:
        ...
        rec["tokens"] = self.tokens_in_current_shard
        rec["bytes_written"] = self.tokens_in_current_shard * UINT16_BYTES
    ...
    def append_tokens(self, token_ids: List[int]) -> Tuple[int, Set[int]]:
        ...
        if arr.min(initial=0) < 0 or arr.max(initial=0) > UINT16_MAX:
        ...
        while cursor < total:
            ...
            take = min(
                total - cursor,
                self.shard_token_capacity - self.tokens_in_current_shard,
                self.rolling_buffer_capacity_tokens - self.rolling_count,
            )

            self.rolling_buffer[self.rolling_count : self.rolling_count + take] = arr16[cursor : cursor + take]
            self.rolling_count += take
            cursor += take

            if (
                self.rolling_count == self.rolling_buffer_capacity_tokens
                or self.tokens_in_current_shard + self.rolling_count >= self.shard_token_capacity
            ):
                self._flush_rolling()
    ...
    def mark_source_file_for_shards(self, shard_indices: Set[int]) -> None:
        ...
        for idx in shard_indices:
            rec = self._ensure_record(idx)
            rec["contributing_source_files"] += 1
            rec["updated_at"] = now
        self._sync_state()
...
def discover_train_bos_eos_files(root: Path) -> Iterator[Path]:
...
def main() -> None:
    ...
    if eos_id is None:
        raise ValueError("[EOS] token missing")
    if bos_id is None:
        raise ValueError("[BOS] token missing")
    ...
    writer = ShardWriter(
        ...
        state,
        int(args.rolling_buffer_mb * 1024 * 1024),
    )

    files = list(discover_train_bos_eos_files(input_root))
    print(f"Found {len(files)} files")
    for idx, path in enumerate(files, start=1):
        rel_key = relative_key(path, input_root)
        if rel_key in completed:
            continue

        in_prog = state.get("in_progress")
        if in_prog and in_prog.get("rel_key") == rel_key:
            start_offset = int(in_prog.get("offset", 0))
            touched_shards = set(in_prog.get("touched_shards", []))
            file_tokens = int(in_prog.get("tokens", 0))
        else:
            start_offset = 0
            touched_shards = set()
            file_tokens = 0
        ...
        logging.info("[%d] %s | split=train | resume_offset=%d", idx, rel_key, start_offset)

        try:
            for checkpoint_offset, chunk in stream_text_chunks_bounded(path, args.chars_per_chunk, start_offset):
                # Preserve BOS/EOS exactly as they already exist in the text.
            ...
            # Do NOT append an extra EOS here, because it is already present in the source text.
        ...
        if size >= args.large_file_threshold_bytes:
